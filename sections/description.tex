...
\end{center}
 to Player with signature $dealer\_signed\_message = ECDSA.Sign(message)$.
	\item Player makes sure that the number L, the game result and new participants' balances are calculated correctly.
	\item Player sends the $message$ and its signature back to Dealer. 
\begin{center}
 $player\_signed\_message = ECDSA.Sign(message)$
\end{center}
\item Dealer 
\end{enumerate}
\end{algorithm}
...
\end{lstlisting}
where $playerSignature = ECDSA.Sign(channelId, round, \\ playerBalance, gameData, seed)$.
...
The contract allocates $t1$ time blocks to Bob so that he could upload a newer state to the channel. After $t1 - 1$ blocks pass Bob restores the connection in time to provide a new state with game results via the $updateChannel$ function. The dispute is removed, Alice and Bob go on gambling.
...
Alice checks the received message, signs it and sends the message back to Bob with data required to close the game.
\\
\\
\begin{tabular}{ccc}
   	\begin{tabular}{c}
